...
if(moduleTitle === "Order Items" || moduleTitle === "Invoice Items" || moduleTitle === "PO Items"){
    return (
        ...
        <div   className="mockup-code bg-secondary text-secondary-content cursor-pointer hover:scale-[1.02] ease-in-out mb-[180px]" >
                  <pre><code>{moduleTitle === "Order Items"?"{{#each order.rentalOrderItems as |item|}}":moduleTitle === "Invoice Items"?"{{#each invoice.invoiceItems as |item|}}":moduleTitle === "PO Items"?"{{#each order.purchaseOrderItems as |item|}}":null}</code></pre>
